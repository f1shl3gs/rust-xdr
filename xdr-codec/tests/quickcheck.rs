...
use xdr_codec::{
    pack_array, pack_opaque_array, padding, unpack_array, unpack_opaque_array, Error, Pack, Unpack,
};
...
fn check_array(arraysz: usize, rxsize: usize, data: Vec<u32>, defl: Option<u32>) -> bool {
    ...
    let rsz = match unpack_array(&mut cur, &mut recv[..], arraysz, defl.as_ref()) {
        ...
        Err(Error::InvalidLen(_)) => return defl.is_none() && recv.len() > arraysz, // expected if recv is too big and there's no default
        Err(e) => {
